...
class RawLLMCall:
    ...
    request_params: dict[str, Any]  # All parameters sent (max_tokens, temperature, etc.)

    # FULL response data
    raw_response: str   # Full raw response text
    response_object: dict[str, Any] | None = None  # Full API response object if available
    ...
    def to_markdown(self) -> str:
        ...
        if self.tool_executions:
            ...
            for i, exe in enumerate(self.tool_executions, 1):
                ...
                lines.extend([
                    ...
                    "",
                    "```json",
                    json.dumps(exe.output_result, indent=2, default=str) if exe.output_result else "null",
                    "```",
                    "",
                ])
        ...
        if self.steps:
            ...
            for step in self.steps:
                ...
                lines.append(f"| `{time_str}` | `{step.step_type}` | {step.description} |")
